...
_WIDGET_INJECT_JS = (
    ...
    "  if (rq) d.setAttribute('data-rqdata', rq);"
...
async def _inject_hcaptcha_widget(page, sitekey: str, rqdata: str = "") -> None:
    """Inject a .h-captcha widget (+ api.js if absent), sitekey + optional rqdata."""
    await page.evaluate(_WIDGET_INJECT_JS, [sitekey, rqdata or ""])
...
async def solve_hcaptcha_realpage(url: str, sitekey: str = None,
    ...
    rqdata — hCaptcha Enterprise extra data (set via data-rqdata on widget).
    ...
    async with _solve_lock:
        async with await cloakbrowser.launch_async(**_browser_kwargs(proxy)) as browser:
            ...
            try:
                ...
                if sitekey:
                    await _inject_hcaptcha_widget(page, sitekey, rqdata or "")
                    await asyncio.sleep(3)
