...
def _solve_puzzle(sitekey: str, timeout_s: int) -> dict:
    ...
    if not data.get("success", True) or "data" not in data:
        return _fail(f"puzzle fetch failed: {json.dumps(data)[:150]}")
    ...
    for i in range(num_puzzles):
        ...
        for b in range(256):
            ...
            for nonce in range(0, 0xFFFFFFFF):
                ...
                hashes += 1
                if struct.unpack("<I", h[:4])[0] < threshold:
                    solutions += bytes(input_128[120:128])
                    found = True
                    break
                if time.monotonic() > deadline:
                    return _fail(f"PoW timeout: solved {i}/{num_puzzles} puzzles")
                if nonce >= 400000:  # safety valve per (b, nonce) sweep
                    break
            if found:
                break
        if not found:
            return _fail(f"puzzle {i} unsolvable within limits")
...
async def solve_friendly(url: str = None, sitekey: str = None,
    ...
    async def _browser_run() -> dict:
        ...
        return _fail("browser widget did not produce a solution")

    try:
        return await asyncio.wait_for(_browser_run(), timeout=max(timeout_s, 40))
    except asyncio.TimeoutError:
        return _fail(f"friendly solve timed out after {timeout_s}s")
    except Exception as exc:
