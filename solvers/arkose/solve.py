...
async def solve_arkose(public_key: str, page_url: str | None = None,
    ...
    try:
        ...
        arkose_domain: str = ""
        arkose_token_js: str = ""
        gt2_denied = False       # {"error":"DENIED ACCESS"} → IP-reputation gate
        gt2_locked = False       # 400 site-lock flavor → key bound to another site
        gt2_status = None
        net_hits: list[str] = []
        ...
        async def on_response(resp):
            ...
            try:
                ...
                if "/fc/gfct" in low or low.rstrip("/").endswith("gfct"):
                    try:
                        body = await resp.text()
                    except Exception:
                        body = ""
                    await _capture_gfct(url, body)
                elif "/fc/ca/" in low:
                    try:
                        ca_result.clear()
                        ca_result.update(json.loads(await resp.text()))
                    except Exception:
                        pass
            except Exception:
                pass
        ...
        # gt2 classification via ROUTE interception — resp.text() in passive listeners
        # races the body stream (empty on some engines); route.fetch() always yields it.
        async def _gt2_route(route):
            try:
                r = await route.fetch()
                body = await r.text()
                _classify_gt2(r.status, body)
        ...
        def _classify_gt2(status: int, body: str):
            nonlocal gt2_denied, gt2_locked, gt2_status
            gt2_status = status
            log.info("arkose: gt2 %s body=%r", status, body[:80])
            if "DENIED ACCESS" in body:
                gt2_denied = True
            elif status >= 400 and ("lock" in body.lower()
                    or ("site" in body.lower() and "invalid" in body.lower())):
                gt2_locked = True

        # NOTE: register before goto — gt2 fires during/just after page load
        await page.route("**/fc/gt2/**", _gt2_route)
  ...
  Start Challenge
        ...
        for i in range(wait_n):
            if gfct_data or arkose_token_js:
                break
            if gt2_denied or gt2_locked:
                break  # gate decided at gt2 — waiting longer cannot recover
        ...
        if not gfct_data:
            ...
            if gt2_denied:
                err = ("arkose gt2 DENIED ACCESS — IP-reputation gate (datacenter/WARP ranges "
                       "blocked before challenge); retry via residential proxy or proxy:'pool'")
            elif gt2_locked:
                err = ("arkose gt2 rejected public_key (site-locked) — run on the real "
                       "site origin for this key (page_url)")
            else:
                err = "no gfct response"
            return {
                "solved": False,
                "error": err,
                "gt2_status": gt2_status,
                "gt2_denied": gt2_denied,
        ...
        for wave in range(max_attempts):
            ...
            try:
                ...
                if answer is None:
                    # ONNX model missing for this variant → Mistral vision fallback
                    # (solves "unknown game type" waves the 24 local models can't)
                    answer = await _vision_answer(img, instruction)
                    if answer is None:
                        log.warning("arkose wave %d: prediction failed (onnx+vision)", wave)
                        continue
                    log.info("arkose wave %d: answer=%d (vision)", wave, answer)
...
async def _vision_answer(img, instruction: str) -> Optional[int]:
    """Mistral-vision fallback for Arkose variants without a local ONNX model.
    The challenge image is the 3x2 option grid — ask for the matching index."""
    try:
        import base64, io
        from pathlib import Path
        from solvers.common.mistral import KeyPool
        keyfile = Path(__file__).resolve().parent.parent / "common" / "apikey.txt"
        pool = KeyPool(str(keyfile))
        buf = io.BytesIO()
        img.convert("RGB").save(buf, "PNG")
        b64 = base64.b64encode(buf.getvalue()).decode()
        text = pool.ask(b64, (
            f"FunCaptcha challenge: '{instruction}'. The image shows 6 numbered "
            "options in a 3x2 grid (row-major: 0,1,2 top row left-to-right; 3,4,5 "
            "bottom row). Reply with ONLY the single index (0-5) of the option "
            "that best satisfies the instruction."), max_keys=6, timeout=30)
        digits = [int(ch) for ch in text if ch.isdigit()]
        if digits and 0 <= digits[0] <= 5:
            return digits[0]
        return None
    except Exception as e:
        log.info("arkose: vision fallback unavailable: %s", str(e)[:80])
        return None
