...
def browser_kwargs(prefix: str, proxy: str | None = None) -> dict:
    ...
    Proxy form: `http://user:pass@host:port` is normalized to Playwright dict
    {server, username, password} so Chromium auth works reliably (cloakbrowser).
    """
    default_headless = "1" if prefix == "TURNSTILE" else "0"
    kw = {"humanize": True,
          "headless": os.getenv("BROWSER_HEADLESS", default_headless) != "0"}
    if proxy:
        kw["proxy"] = _normalize_proxy(proxy)
    if os.getenv(f"{prefix}_GEOIP") == "1":
        # cloakbrowser geoip=True needs geoip2; never hard-crash the whole solve
        try:
...
def _normalize_proxy(proxy: str | dict) -> str | dict:
    """Convert http://user:pass@host:port → {server, username, password} when needed."""
    ...
    if not p.hostname:
        return proxy
    # no userinfo → leave string
    if p.username is None and p.password is None:
        return proxy
    port = f":{p.port}" if p.port else ""
    server = f"{p.scheme or 'http'}://{p.hostname}{port}"
    return {
        "server": server,
