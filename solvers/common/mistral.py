...
class KeyPool:
    def __init__(self, keyfile: str, model: str = _DEFAULT_MODEL,
        ...
        keys = [k.strip() for k in raw if k.strip() and not k.strip().startswith("#")]
        # dedupe, preserve order
        seen = set()
        self.keys = [k for k in keys if not (k in seen or seen.add(k))]
        # Empty pool allowed at construct time (ONNX-only recaptcha path).
        # classify/ask still raise if no keys when vision is actually needed.
        self.model = model
        ...
        if self.keys:
            ...
            self._cursor = itertools.cycle([""])  # dummy; _next_live_key will raise
    ...
    def _next_live_key(self) -> str:
        with self._lock:
            if not self.keys:
                raise RuntimeError(
                    "no Mistral keys in common/apikey.txt — image challenges need keys "
                    "or classifier=yolo with ONNX model")
            now = time.monotonic()
