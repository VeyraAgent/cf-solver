...
_DEFAULT_THRESHOLD = 0.42  # lower = more recall on ambiguous tiles (pure-local YOLO)
...
_TARGET_ALIASES = {
    ...
    "automobile": "Car", "automobiles": "Car",
    ...
    "street light": "Traffic Light", "stoplight": "Traffic Light",
}
...
class OnnxClassifier:
    ...
    def classify(self, image_b64: str, target: str,
        ...
        try:
            ...
            with self._lock:
                out = self.sess.run(None, {self.inp: x})[0][0]
            # soft: score for wanted class even if not argmax (ambiguous tiles)
            name_to_idx = {v: int(k) for k, v in self.names.items()} if isinstance(
                next(iter(self.names.keys())), int) else {v: i for i, v in self.names.items()}
            # names may be dict int->str or list-like from meta
            if not name_to_idx:
                name_to_idx = {self.names[i]: i for i in range(len(self.names))}
            idx = name_to_idx.get(want)
            if idx is None:
                # names keys might be str
            ...
            pred = self.names[pred_idx] if pred_idx in self.names else self.names.get(str(pred_idx))
            # accept if wanted class is strong enough OR argmax matches with lower bar
            if conf >= self.threshold:
                return True
            if pred == want and conf >= (self.threshold * 0.75):
                return True
            return False
...
class HybridClassifier:
    """ONNX-first with Mistral on miss (or unknown target).

    Known classes: try local ONNX first (fast). If ONNX says no-match, still ask
    Mistral — YOLO alone misses many reCAPTCHA tiles. Unknown targets go straight
    to Mistral.
    """
    ...
    def classify(self, image_b64: str, target: str,
                 max_keys: int = 8, timeout: int = 40) -> bool:
        known = _normalize_target(target) is not None
        if known:
            if self.onnx.classify(image_b64, target, max_keys, timeout):
                return True
            # ONNX miss → Mistral second opinion (keys present)
