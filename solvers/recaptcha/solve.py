...
from solvers.common.browser import browser_kwargs, run_pre_actions, run_post_fetch, route_glob
...
_solve_lock = asyncio.Semaphore(2)  # allow 2 parallel recaptcha browsers (was Lock → queue burns timeout)
...
<script src="https://www.google.com/recaptcha/__LIB__?render=__SITEKEY__&onload=__onRc" async defer></script>
</head><body><div id="out">waiting</div></body></html>"""


# Invisible v2 page: size=invisible widget + grecaptcha.execute() (NOT v3 render=sitekey).
    ...
    try {
      ...
      gre.ready(function () {
        try {
          ...
          // some embeds accept bare execute after explicit render fails
          Promise.resolve(gre.execute("__SITEKEY__"))
            .then(function (t) { window.__token = t || ""; })
            .catch(function (e2) { window.__err = String(e2); });
        }
      });
    } catch (e) { window.__err = String(e); }
  };
</script>
<script src="https://www.google.com/recaptcha/api.js?onload=__onRc&render=explicit" async defer></script>
</head><body><div id="rc"></div></body></html>"""
...
async def _solve_via_execute(sitekey: str, url: str, action: str,
                             enterprise: bool = False, proxy: str = None,
                             invisible: bool = False) -> dict:
    """Mint a token via grecaptcha[.enterprise].execute() on a route-intercepted page.

    invisible=True → v2 invisible widget template (size=invisible).
    invisible=False → v3 score template (render=sitekey + action).
    """
    t0 = time.monotonic()
    if invisible and not enterprise:
    ...
    async with _solve_lock:
        async with await cloakbrowser.launch_async(**_browser_kwargs(proxy)) as browser:
            ...
            try:
                ...
                # wait for script + execute (was 20s — often short under proxy)
                for _ in range(50):
...
async def solve_recaptcha_invisible(sitekey: str, url: str, action: str = "submit",
    ...
    """Solve invisible reCAPTCHA v2 via size=invisible widget + execute()."""
...
async def solve_recaptcha_invisible_realpage(
    ...
    async with _solve_lock:
        try:
            browser_cm = await cloakbrowser.launch_async(**_browser_kwargs(proxy))
        except Exception as e:
            return {"error": f"browser launch failed: {e}",
                    "elapsed": round(time.monotonic() - t0, 1), "proxy": bool(proxy)}
        async with browser_cm as browser:
            ...
            try:
                try:
                    ...
                    return {"error": f"goto failed: {str(e).splitlines()[0][:160]}",
                            "elapsed": round(time.monotonic() - t0, 1),
                            "proxy": bool(proxy)}
                if pre_actions:
                    await run_pre_actions(page, pre_actions)
                ...
                # Official demos (appspot) fire invisible on form submit — click first.
                for sel in (
                    "button[type=submit]",
                    "input[type=submit]",
                    "button.g-recaptcha",
                    ...
                    try:
                        ...
                        if await loc.count() > 0:
                            ...
                            log.info("invisible realpage: clicked %s", sel)
                            break
                    except Exception:
                        pass
                await asyncio.sleep(1.2)

                token0 = await page.evaluate(
                    "() => window.__rc_token"
                    " || document.querySelector('#g-recaptcha-response')?.value"
                    " || document.querySelector('textarea[name=\"g-recaptcha-response\"]')?.value"
                    " || ''")
                if token0 and len(token0) > 40:
                    return {"token": token0, "action": action,
                            "elapsed": round(time.monotonic() - t0, 1),
                ...
                deadline = time.monotonic() + max(30, int(timeout_s or 90))
                last_err = ""
                image_attempts = 0
...
async def solve_recaptcha_v2(sitekey: str, url: str,
    ...
    # Prefer hybrid (ONNX + Mistral) when keys exist; yolo-only only if forced
    if not classifier or classifier == "auto":
        from .onnx_classifier import get_classifier
        has_onnx = get_classifier() is not None
        # keys in apikey.txt?
        keys = []
        if _KEYFILE.exists():
            keys = [ln.strip() for ln in _KEYFILE.read_text().splitlines()
                    if ln.strip() and not ln.strip().startswith("#")]
        if has_onnx and keys:
            classifier = "hybrid"
        elif has_onnx:
            classifier = "yolo"
        else:
    ...
    async with _solve_lock:
        async with await cloakbrowser.launch_async(**_browser_kwargs(proxy)) as browser:
            ...
            try:
                await page.route(
                    route_glob(url),
                    lambda r: r.fulfill(body=page_data, status=200, content_type="text/html"),
                )
                await page.goto(url, wait_until="domcontentloaded", timeout=45000)
                # wait for recaptcha iframe
                try:
                    await page.wait_for_selector(_ANCHOR_IFRAME, timeout=20000)
                except Exception:
                ...
                for attempt in range(1, max_attempts + 1):
                    ...
                    for _ in range(30):
                        ...
                        if await _find_frame(page, "/bframe") and \
                            ...
                            challenge = True
                            break
                        # also detect image tiles without table yet
                        if await _find_frame(page, "/bframe"):
                            try:
                                if await page.frame_locator(_BFRAME_IFRAME).locator(
                                        "#rc-imageselect, .rc-imageselect-target").count() > 0:
                                    challenge = True
                                    break
                            except Exception:
                                pass
                    ...
                    if challenge:
                        ...
                        # poll for token / checked after verify
                        for _ in range(12):
                            await asyncio.sleep(1)
                            token = await _get_token(page)
                            if token:
                            ...
                            if "recaptcha-checkbox-checked" in await _anchor_class(page):
                                ...
                                if token:
                                    return {"token": token, "attempts": attempt,
                                            "elapsed": round(time.monotonic() - t0, 1),
                                            "method": "image", "solved": True}
                            # still in challenge? try one more image pass this attempt
                            if await _find_frame(page, "/bframe"):
                                try:
                                    if await page.frame_locator(_BFRAME_IFRAME).locator(
                                            "table").count() > 0:
                    ...
                    if attempt < max_attempts:
                        # reload page for clean challenge on next attempt
                        try:
                            await page.reload(wait_until="domcontentloaded", timeout=30000)
                            await page.wait_for_selector(_ANCHOR_IFRAME, timeout=15000)
                        except Exception:
                            await asyncio.sleep(3 * attempt)

                return {"error": f"failed after {max_attempts} attempts",
                        "elapsed": round(time.monotonic() - t0, 1), "solved": False}
            finally:
                await page.close()
