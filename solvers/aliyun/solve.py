...
TYPE_CHECK_BOX = "CHECK_BOX"   # AutoClaw (scene 18vhnjxl): checkbox → captchaVerifyCallback directly
...
async def _verify_in_page(page, tok: dict, scene_id: str, prefix: str, region: str = "sgp") -> str:
    """Call VerifyCaptchaV3 from inside the page (same origin/proxy). Returns VerifyCode."""
    if region == "ga":
        host = f"https://{prefix}.captcha-open-ga-web.aliyuncs.com/"
    else:
...
async def solve_aliyun(scene_id: str, prefix: str, region: str = "sgp",
    ...
    try:
        ...
        async def _attempt_puzzle(attempt, type_name):
            ...
            code = await _verify_in_page(page, token, scene_id, prefix, region)
        ...
        async def _attempt_traceless(attempt, type_name):
            ...
            for i in range(40):
                ...
                if tok:
                    break
                if i == 12:
                    # re-click once — CHECK_BOX sometimes needs a second signal
                    await cdp.send("Input.dispatchMouseEvent", {
                        "type": "mousePressed", "x": box["x"] + box["w"] / 2,
                        "y": box["y"] + box["h"] / 2, "button": "left", "clickCount": 1})
                    await cdp.send("Input.dispatchMouseEvent", {
                        "type": "mouseReleased", "x": box["x"] + box["w"] / 2,
                        "y": box["y"] + box["h"] / 2, "button": "left", "clickCount": 1})
        ...
        async def _attempt_oneclick(attempt, type_name):
            # ONE_CLICK / CHECK_BOX: popup opens first, then click the real checkbox.
            await page.evaluate("()=>{window.__verify=null;}")
            await open_popup()
            # wait for the checkbox element inside the popup
            box = None
            for _ in range(16):
                box = await page.evaluate(
                    "()=>{const sel=['#aliyunCaptcha-checkbox-element','#aliyunCaptcha-checkbox-body',"
                    "'#aliyunCaptcha-checkbox-wrapper','#aliyunCaptcha-btn'];"
            ...
            if not box:
                log.info("aliyun attempt %d [%s]: checkbox never rendered", attempt, type_name)
                return None
            await page.wait_for_timeout(400 + int(300 * (attempt % 2)))  # human-ish pause
            for t in ("mousePressed", "mouseReleased"):
                await cdp.send("Input.dispatchMouseEvent", {
            ...
            for _ in range(24):
                ...
                await page.wait_for_timeout(250)
            if not tok:
                log.info("aliyun attempt %d [%s]: checkbox clicked, no callback", attempt, type_name)
                return None
            token = tok if isinstance(tok, dict) else json.loads(tok)
            code = await _verify_in_page(page, token, scene_id, prefix, region)
            log.info("aliyun attempt %d [%s]: -> %s", attempt, type_name, code)
        ...
        elif ct == TYPE_ONE_CLICK or ct == TYPE_CHECK_BOX:
