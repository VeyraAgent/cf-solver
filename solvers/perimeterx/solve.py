...
def _result(t0, ev, ua=None, cookies=None, error=None) -> dict:
    ...
    solved = bool(px3 and (ev.get("actuated") or rotated or not ev.get("gate_reached")))
    # Silent-pass / pre-minted _px3 still usable for replay even if press-hold UI never showed.
    if px3 and not solved:
        solved = True
    out = {
        "solved": solved,
        "token": px3,  # normalize for clients expecting token
        "px3": px3,
        "cookies": cookies,
    ...
    if error:
        out["error"] = error
    elif not ev.get("gate_reached") and not px3:
